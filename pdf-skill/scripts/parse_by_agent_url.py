...
def create_agent_task(url: str, file_name: Optional[str] = None, language: str = "ch",
                     page_range: Optional[str] = None, enable_table: bool = True,
                     is_ocr: bool = False, enable_formula: bool = True) -> str:
    """
    创建 Agent 轻量解析任务
    ...
    Returns:
        task_id: 任务 ID
    """
    api_url = "https://mineru.net/api/v1/agent/parse/url"
...
def main():
    parser = argparse.ArgumentParser(
        description="使用 MinerU Agent 轻量 API 解析远程文档（免登录）",
...
示例:
  # 基本用法
  python parse_by_agent_url.py https://example.com/small.pdf
  
  # 指定参数
  python parse_by_agent_url.py https://example.com/doc.pdf \\
    --language en \\
    --page_range "1-10" \\
    --output_dir ./output
        """
    ...
    parser.add_argument("--file_name", help="文件名（用于判断文件类型）")
    ...
    try:
        print(f"使用 Agent 轻量 API 解析: {args.url}")
        ...
        if args.output_name:
            ...
            base_name = get_filename_from_url(args.url)
            if args.file_name:
                base_name = sanitize_filename(os.path.splitext(args.file_name)[0])
